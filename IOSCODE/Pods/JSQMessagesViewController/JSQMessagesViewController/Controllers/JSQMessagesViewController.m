...
- (void)jsq_configureMessagesViewController
{
    ...
    self.viewBack = [[UIView alloc] initWithFrame: CGRectMake (0, 17, 227, 55)];
    [self.viewBack setBackgroundColor:[UIColor cyanColor]];
    [self.view addSubview:self.viewBack];
    
    UIView *viewTop = [[UIView alloc] initWithFrame: CGRectMake (0, 72, [[UIScreen mainScreen] bounds].size.width, 2)];
    [viewTop setBackgroundColor:[UIColor whiteColor]];
    [self.view addSubview:viewTop];
    
    UIImageView *back =[[UIImageView alloc] initWithFrame:CGRectMake(8,19,21,20)];
    back.image=[UIImage imageNamed:@"back"];
    [self.viewBack addSubview:back];
    
    _gpNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(33, 13, 227, 19)];
    //_gpNameLabel.font=[UIFont boldSystemFontOfSize:16.0];
    _gpNameLabel.font=[UIFont fontWithName:@"Ubuntu" size:16.0f];
    _gpNameLabel.textColor=[UIColor whiteColor];
    _gpNameLabel.backgroundColor=[UIColor clearColor];
    _gpNameLabel.adjustsFontSizeToFitWidth = YES;
    [self.viewBack addSubview:_gpNameLabel];
    
    _timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(33, 31, 227, 17)];
    _timeLabel.font=[UIFont fontWithName:@"Ubuntu" size:10.0f];
    _timeLabel.textColor=[UIColor whiteColor];
    // _timeLabel.backgroundColor=[UIColor clearColor];
    [self.viewBack addSubview:_timeLabel];
    
    UIImageView *dot =[[UIImageView alloc] initWithFrame:CGRectMake(0,0,[[UIScreen mainScreen] bounds].size.width,[[UIScreen mainScreen] bounds].size.height)];
    dot.image=[UIImage imageNamed:@"bkg"];
    
    [self.view addSubview:dot];
    [self.view sendSubviewToBack:dot];
    
    _imgParticipants = [[UIImageView alloc] initWithFrame:CGRectMake([[UIScreen mainScreen] bounds].size.width - 50,27,37,37)];
    _imgParticipants.image = [UIImage imageNamed:@"group"];
    [self.view addSubview:_imgParticipants];
    ...
    self.inputToolbar.contentView.textView.font = [UIFont fontWithName:@"Ubuntu" size:16.0f];
...
- (UICollectionViewCell *)collectionView:(JSQMessagesCollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!isMediaMessage) {
        //cell.textView.backgroundColor = [UIColor lightGrayColor];
        cell.textView.textColor = [UIColor whiteColor];
       // NSLog(@"%@",[messageItem text]);
        cell.textView.text = [messageItem text];
       // NSLog(@"%@",cell.textView.text);
        cell.textView.font = [UIFont fontWithName:@"Ubuntu" size:16.0f];
        if ([UIDevice jsq_isCurrentDeviceBeforeiOS8]) {
